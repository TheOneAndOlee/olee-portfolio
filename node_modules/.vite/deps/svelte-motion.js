...
import {
  ...
  velocityPerSecond,
  wrap
} from "./chunk-D3JWJVHV.js";
import {
  cancelSync,
  es_default,
  flushSync,
  getFrameData
} from "./chunk-RIF7CQAF.js";
import {
  invariant,
  warning
} from "./chunk-M7TPFSYX.js";
import {
  alpha,
  color,
  complex,
...
function UseVisualElement($$anchor, $$props) {
  ...
  afterUpdate(() => {
    tick().then(() => {
      get(visualElement2).animationState?.animateChanges();
    });
  });
  onDestroy(() => {
    get(visualElement2)?.notifyUnmount();
  });
    ...
    () => {
      if (!get(visualElementRef) && createVisualElement()) {
        set(visualElementRef, createVisualElement()(Component(), {
          ...
          blockInitialAnimation: strict_equals($presenceContext()?.initial, false)
        }));
      }
    }
  );
  ...
  legacy_pre_effect(
    ...
    () => {
      if (get(visualElement2)) {
        ...
        mutate(visualElement2, get(visualElement2).isPresenceRoot = !get(parent) || strict_equals(get(parent).presenceId, $presenceContext()?.id, false));
...
function makeLatestValues(props, context, presenceContext, scrapeMotionValues) {
  const values = {};
  const blockInitialAnimation = strict_equals(presenceContext?.initial, false);
...
function UseTapGesture($$anchor, $$props) {
  ...
  function onPointerUp(event, info) {
    ...
    !isNodeOrChild(visualElement2().getInstance(), event.target) ? get(onTapCancel)?.(event, info) : get(onTap)?.(event, info);
  }
  function onPointerCancel(event, info) {
    if (!checkPointerEnd()) return;
    get(onTapCancel)?.(event, info);
  }
...
function Exit($$anchor, $$props) {
  ...
  const effect = (pres) => {
    ...
    const animation = visualElement2().animationState?.setActive(AnimationType.Exit, !isPresent2, { custom: $presenceContext()?.custom ?? get(custom) });
        ...
        default: ($$anchor2, $$slotProps) => {
          ...
          var node_1 = first_child(fragment_1);
          {
            ...
            element(
              ...
              false,
              ($$element, $$anchor3) => {
                action($$element, ($$node) => get(motion2)?.($$node));
